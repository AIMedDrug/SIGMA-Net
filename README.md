...
To enhance feature representation, we developed an SIGMA-Net (SIamese structure and Graph-aware Multistructural Affinity prediction Network) model, which utilizes Siamese networks to capture distinct structural features: ligand graphs, protein contact maps, and protein-ligand interaction graphs.
...
![dataset Image](https://github.com/AIMedDrug/SIGMA-Net/blob/main/notebook/top5/augmentedPairwiseDataset.png?raw=true)

## The structure representation of multimodal fusion deep learning model

The following is the architecture of the SIGMA-Net model. The reference data and the AF2-generated data are processed by the Siamese network to capture the feature difference. Protein contact maps and ligand graphs are processed by MPNN, and protein-ligand interaction graphs are processed by GIGN.

![model](https://github.com/AIMedDrug/SIGMA-Net/blob/main/notebook/top5/STGNet_model.png?raw=true)

## Structure and graph-aware Siamese network model protocol 

The steps required for SIGMA-Net are as follows:  (1) Build the augmented pairwise dataset. (2) Generate molecular maps, protein contact maps, and protein-ligand interaction maps for paired data; and split the data into training, validation, and test sets. (3) Train the model using Siamese networks for class three graphs.

Be aware: SIGMA-Net also can be used in other pairwise learning task.

We provide scripts for data processing and model training. If you are interested in SIGMA-Net, you can use the .py and .ipynb files.
...
github address: https://github.com/AIMedDrug/SIGMA-Net.git

**2 Google colab's .ipynb file address for testing**

(1) Direct Testing model

STGNet_test: https://colab.research.google.com/github/AIMedDrug/SIGMA-Net/blob/main/notebook/top5/STGNet_test.ipynb

(2) Gain predicted values for different TKIs

STGNet_getPredValue: https://colab.research.google.com/github/AIMedDrug/SIGMA-Net/blob/main/notebook/top5/STGNet_getPredValue.ipynb

(3) Evaluation: Obtain RMSE, MAE, Pearson coefficients, and Spearman coefficients for different TKIs

STGNet_pltMetrics: https://colab.research.google.com/github/AIMedDrug/SIGMA-Net/blob/main/notebook/top5/STGNet_pltMetrics.ipynb

## Other information

We have provided the relevant code in the SIGMA-Net/script directory on GitHub:
...
2. ContactMap map features
   ...
   |          | Mutated single conformation pdb file generated by AlphaFold2. |
    ...
    In the case folder, we provided a compressed package data.rar. The data needed are in the compressed package, and the content inside is as the following directory structure. The needed folders are labeld by *.
...
```
    ...
    │   ├── contact *
    │   └── data_raw *
    ├── dockEnergyFilterModify
    ├── exp
    │   ├── ABL_AlphaFold *
    │   ├── autoDockScript_small
    │   └── pocket *
    └── multiMutation_top2_twin_muts_tuple_inter
        ├── axitinib
        ├── bosutinib
        ├── dasatinib
        ├── imatinib
